...
<div class="top"><div><h1>Germany Student Jobs</h1><p class="sub">Computer-science opportunities across Germany; part-time and minijob listings are restricted to Leipzig and nearby towns.</p></div><div class="updated" id="updated"></div></div>
<div class="tabs" aria-label="Job category"><button class="tab active" data-category="cs" onclick="setCategory('cs')">CS & Tech</button><button class="tab" data-category="part-time" onclick="setCategory('part-time')">Part-time & Minijob · Leipzig area</button></div>
...
function render(){const q=document.querySelector('#search').value.toLowerCase(),type=document.querySelector('#type').value,status=document.querySelector('#status').value;
 ...
 document.querySelector('#jobs').innerHTML=rows.length?rows.map(j=>{const st=getState(j.id),label=(j.category||'cs')==='cs'?'CS & Tech':'Part-time & Minijob · Leipzig area';return `<article class="card"><div class="row"><div><a class="title" href="${esc(j.url)}" target="_blank" rel="noreferrer">${esc(j.title)}</a><div class="company">${esc(j.company)}</div></div><div class="score">${j.score}</div></div><p class="meta">${esc(j.location)} · ${esc(label)} · ${esc(j.role_type)} · ${esc(j.source)} · ${esc(st)}</p><p class="reasons">${j.reasons.map(esc).join(' · ')}</p><div class="actions">${['saved','applied','dismissed'].map(s=>`<button class="${st===s?'active':''}" onclick="setState('${esc(j.id)}','${s}')">${s[0].toUpperCase()+s.slice(1)}</button>`).join('')}<a class="button" href="${esc(j.url)}" target="_blank" rel="noreferrer">Apply ↗</a></div></article>`}).join(''):'<div class="empty">No jobs match these filters.</div>'}
...
def telegram_message(
    ...
    for job in new_jobs[:limit]:
        category = (
            "CS & Tech" if job.category == "cs" else "Part-time & Minijob · Leipzig area"
        )
        lines.extend([
            f"• {job.title}",
