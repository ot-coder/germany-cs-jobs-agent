...
<title>Germany CS Jobs Agent</title><style>
...
.controls{display:grid;grid-template-columns:minmax(220px,1fr) auto auto;gap:10px;margin:28px 0 14px}input,select{background:#111925;border:1px solid var(--line);border-radius:12px;color:#e8eef7;padding:12px 14px;font:inherit}
...
<div class="controls"><input id="search" type="search" placeholder="Search title, company, city…"><select id="type"><option value="">All role types</option><option value="werkstudent">Werkstudent</option><option value="entry-level">Entry level</option></select><select id="status"><option value="">All statuses</option><option value="new">New</option><option value="saved">Saved</option><option value="applied">Applied</option><option value="dismissed">Dismissed</option></select></div>
...
function render(){const q=document.querySelector('#search').value.toLowerCase(),type=document.querySelector('#type').value,status=document.querySelector('#status').value;
 const rows=all.filter(j=>(!q||`${j.title} ${j.company} ${j.location}`.toLowerCase().includes(q))&&(!type||j.role_type===type)&&(!status||getState(j.id)===status));
 ...
 document.querySelector('#jobs').innerHTML=rows.length?rows.map(j=>{const st=getState(j.id);return `<article class="card"><div class="row"><div><a class="title" href="${esc(j.url)}" target="_blank" rel="noreferrer">${esc(j.title)}</a><div class="company">${esc(j.company)}</div></div><div class="score">${j.score}</div></div><p class="meta">${esc(j.location)} · ${esc(j.role_type)} · ${esc(j.source)} · ${esc(st)}</p><p class="reasons">${j.reasons.map(esc).join(' · ')}</p><div class="actions">${['saved','applied','dismissed'].map(s=>`<button class="${st===s?'active':''}" onclick="setState('${esc(j.id)}','${s}')">${s[0].toUpperCase()+s.slice(1)}</button>`).join('')}<a class="button" href="${esc(j.url)}" target="_blank" rel="noreferrer">Apply ↗</a></div></article>`}).join(''):'<div class="empty">No jobs match these filters.</div>'}
...
def telegram_message(new_jobs: list[Job], site_url: str, limit: int = 8) -> str:
    ...
    for job in new_jobs[:limit]:
        lines.extend([f"• {job.title}", f"  {job.company} · {job.location} · {job.score}/100", f"  {job.url}", ""])
