        ...
        with self._connect() as connection:
            connection.execute("""
                CREATE TABLE IF NOT EXISTS jobs (
                    ...
                    source TEXT NOT NULL, role_type TEXT NOT NULL, remote INTEGER NOT NULL,
        ...
        with self._connect() as connection:
            for job in jobs:
                ...
                if not exists:
                    inserted += 1
                connection.execute("""
                    INSERT INTO jobs (id,title,company,location,url,description,source,role_type,remote,published_at,score,reasons,first_seen_at)
                    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)
                    ON CONFLICT(id) DO UPDATE SET
                        title=excluded.title, company=excluded.company, location=excluded.location,
                        url=excluded.url, description=excluded.description, source=excluded.source,
                        role_type=excluded.role_type, remote=excluded.remote,
                        published_at=excluded.published_at, score=excluded.score, reasons=excluded.reasons
                """, (
                    job.id, job.title, job.company, job.location, job.url, job.description,
                    job.source, job.role_type, int(job.remote), job.published_at, job.score,
